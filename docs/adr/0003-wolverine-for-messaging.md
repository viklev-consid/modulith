# ADR-0003: Wolverine for Messaging, Outbox, and Delayed Messages
...
3. Delayed follow-up work that should participate in the same handler/messaging model.

Historically these are separate libraries: MediatR for mediation, MassTransit for messaging with outbox, and Hangfire or Quartz for scheduled jobs. Each has its own conventions, configuration, and learning curve. Wiring them together is a source of friction and bugs (e.g., ensuring events are published after the transaction commits).

Wolverine (part of the JasperFx suite, alongside Marten) provides mediation, message handling, delayed messages, and transactional outbox behavior in a single library with a consistent model. It uses source generation for handler discovery (no reflection-heavy startup), integrates with EF Core for the outbox, and exposes `IMessageBus` as the single dispatch surface.

ADR-0032 later split recurring, operator-visible scheduled jobs out to TickerQ. This ADR still owns Wolverine's role as the application dispatch and durable messaging surface.
...
- Delayed messages and transactional follow-up work that belongs to the message pipeline
...
Do not use MediatR, MassTransit, or Hangfire in addition. For recurring cron-style scheduled jobs, use TickerQ per ADR-0032 and dispatch module-owned Wolverine commands from the TickerQ trigger.

## Consequences

**Positive:**

- One dispatch and messaging model for commands, queries, events, delayed messages, and handler middleware.
...
- Delayed message support remains available when timing is part of a transactional workflow.
...
- Recurring jobs now have a second tool in the system. ADR-0032 narrows that tool to scheduling only; Wolverine remains the place where module behavior runs.
...
- ADR-0032 (TickerQ for Recurring Scheduled Jobs): TickerQ owns recurring schedules and dispatches Wolverine commands.

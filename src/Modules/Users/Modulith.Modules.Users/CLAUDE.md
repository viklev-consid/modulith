...
14. `ExternalLogin` is unique per `(Provider, Subject)` globally — one Google account can only be linked to one user. It is also unique per `(User, Provider)` — a user can have at most one Google account linked at a time. Both constraints are enforced at the aggregate level (`LinkExternalLogin` rejects any second login for the same provider, regardless of subject) and at the database level (`ix_external_logins_provider_subject` and `ix_external_logins_user_id_provider`).
15. Credential-retention guardrail: `UnlinkExternalLogin` fails if the user has no other credential (password or another provider's external login). With only one provider (Google), a password-less user must set a password before unlinking Google.
...
- **Don't add `ExpiresAt > now` back to the Step 1 reuse check in `GoogleLoginHandler`.** The partial unique index `(provider, subject) WHERE consumed_at IS NULL` covers expired rows too — an expired-but-unconsumed row still blocks any INSERT for the same subject. If Step 1 skips expired rows, the INSERT in Step 3 collides, the `DbUpdateException` catch block returns 202 silently, and no email is ever sent. The handler must refresh (not skip) expired unconsumed rows.

- **Don't widen `LinkExternalLogin`'s duplicate check back to `(provider, subject)`.** The guard is intentionally provider-level: a user can have at most one Google account. Narrowing it to the exact subject pair re-opens the backdoor credential attack — a second Google subject linked to the same user is undetectable to the owner (the unlink endpoint targets provider only, not subject) and leaves a hidden persistent credential after unlink removes the legitimate one.
- **Don't modify `GoogleLoginHandler` (fast path) or `UnlinkGoogleLoginHandler` without preserving the `FOR UPDATE` lock and the publish-before-commit order.** Both handlers hold a `SELECT … FOR UPDATE` on the same `external_logins` row — login by `(provider, subject)`, unlink by `(user_id, provider)`. Both handlers rely on Wolverine's `AutoApplyTransactions()` middleware to manage the transaction lifecycle. The required sequence is: `FOR UPDATE` lock → state mutation → `SaveChangesAsync` → `bus.PublishAsync` → (Wolverine commits). The `PublishAsync` call enqueues the outbox message into the Wolverine outbox table within the same database transaction, so the message and the domain change commit atomically. **Do not add explicit `BeginTransactionAsync` / `CommitAsync` calls inside these handlers** — Wolverine's middleware has already started a transaction and calling `BeginTransactionAsync` on an already-open EF Core connection throws `InvalidOperationException`. The lock is held from the `FOR UPDATE` query until Wolverine commits after the handler returns, which is the correct window.

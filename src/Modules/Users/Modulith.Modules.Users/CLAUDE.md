...
- **Don't modify `GoogleLoginHandler` (fast path) or `UnlinkGoogleLoginHandler` without preserving the `FOR UPDATE` lock and the publish-before-commit order.** Both handlers hold a `SELECT … FOR UPDATE` on the same `external_logins` row — login by `(provider, subject)`, unlink by `(user_id, provider)`. The required sequence inside the explicit transaction is: `FOR UPDATE` lock → state mutation → `SaveChangesAsync` → `bus.PublishAsync` → `CommitAsync`. The `PublishAsync` call enqueues the outbox message into the Wolverine outbox table within the same database transaction, so the message and the domain change commit atomically. Calling `CommitAsync` before `PublishAsync` breaks that guarantee — any failure between commit and publish leaves durable auth state (unlinked account + revoked tokens, or issued refresh token) with no corresponding downstream event. Removing either lock, collapsing them into separate transactions, or re-ordering the lock and the state read re-opens the race where login can mint a valid session after unlink completed.
